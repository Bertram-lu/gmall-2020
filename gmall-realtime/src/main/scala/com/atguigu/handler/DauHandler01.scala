...
object DauHandler01 {
    ...
    //使用Redis对数据做跨批次去重
    def filterByRedis(startUpLogDString : DStream[StartUpLog] , sc : SparkContext) : DStream[StartUpLog] = {
        /*//方案一:每一条数据都会获取连接并释放
        val value1: DStream[StartUpLog] = startUpLogDString.filter(
            startUpLog => {
                //获取Redis连接
                val jedisClient: Jedis = RedisUtil.getJedisClient
                //查询Redis中是否存在该mid
                val redisKey = s"dau:${startUpLog.logDate}"
                val boolean: Boolean = !jedisClient.sismember(redisKey, startUpLog.mid)
                //回收Redis连接
                jedisClient.close()
                //返回结果
                boolean
        ...
        /*//方案二：每个分区获取一次连接
        val value2: DStream[StartUpLog] = startUpLogDString.mapPartitions(
            iter => {
                //获取Redis连接
                val jedisClient: Jedis = RedisUtil.getJedisClient
                //过滤
        ...
        //1---->Driver端,全局只执行一次
        val value3: DStream[StartUpLog] = startUpLogDString.transform(
            //2---->Driver端,每个批次执行一次
            rdd => {
                //a.获取连接
                val jedisClient: Jedis = RedisUtil.getJedisClient
